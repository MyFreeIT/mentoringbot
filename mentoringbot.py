...
class MentorChatBot:
    ...
    async def send_welcome(self, message: types.Message):
        ...
        Отправляет приветственное сообщение:
          ...
          - Если пользователь (студент) уже зарегистрирован, отображается кнопка "Вызвать ментора".
    ...
    async def call_mentor(self, callback_query: types.CallbackQuery):
        ...
        Пользователь всегда добавляется в очередь ожидания, если его там ещё нет.
        """
        ...
        if user_id not in self.waitlist:
            ...
            await self.bot.send_message(
                self.mentor_id,
                f"⚡ Участник {username} ({user_id}) добавлен в очередь ожидания. Используйте кнопку 'Присоединиться' или команду /join."
            )
        else:
    ...
    async def join_chat(self, message: types.Message):
        ...
        Используется только очередь waitlist.
        """
        if message.from_user.id != self.mentor_id:
