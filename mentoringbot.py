...
class MentorChatBot:
    ...
    def register_handlers(self):
        ...
        Обработчики:
         ...
         - Для участников: если username отсутствует, предлагается выбрать "Войти как Аноним" или "Ввести имя".
        ...
        # Обработка ввода пароля для участников:
        ...
        # Если участник уже зарегистрирован, выводим сообщение об ожидании
        self.dp.message.register(
        ...
        # Inline кнопки для mensora: пересылка сообщений – пропускаем команды (начинающиеся со слеша)
        self.dp.message.register(
    ...
    async def send_welcome(self, message: types.Message):
        ...
        Иначе – отправляется кнопка "Войти".
        """
    ...
    async def check_password(self, message: types.Message):
        ...
        Если пароль корректен:
          – для менторов: регистрирует их без запроса (поскольку /start выдаёт меню).
          – для участников: если username отсутствует, предлагает выбрать способ регистрации.
        """
        if message.text == self.access_password:
            if message.from_user.id == self.mentor_id:
                # Регистрация ментор – здесь этот обработчик не срабатывает, т.к. для ментора он не регистрируется
                self.users[
    ...
    async def set_custom_name(self, message: types.Message):
        """
        Использует введённый текст в качестве имени для пользователя, находящегося в режиме ввода.
        """
    ...
    async def waiting_message(self, message: types.Message):
        """
        Сообщает, что участник уже зарегистрирован и ждёт подключения ментора.
        """
    ...
    async def call_mentor(self, callback_query: types.CallbackQuery):
        ...
        Если сессия активна, добавляет участника в очередь ожидания.
        """
    ...
    async def join_chat(self, message: types.Message):
        """
        Команда для менторов (через слэш или, через inline, посредством вызова fake-сообщения):
        Подключение к ожидающему участнику. Если сессия активна – просит завершить текущий чат.
    ...
    async def mentor_join(self, callback: types.CallbackQuery):
        """
        Обработчик inline-кнопки "Присоединиться" для ментора.
        Использует callback.from_user для проверки прав.
        """
        if callback.from_user.id != self.mentor_id:
            await callback.answer("❌ У вас нет прав ментора!", show_alert=True)
            return

        # Создаем временный объект, чтобы передать его в join_chat
        class FakeMessage:
            pass
    ...
    async def mentor_end(self, callback: types.CallbackQuery):
        """
        Обработчик inline-кнопки "Завершить чат" для ментора.
        Использует callback.from_user для проверки прав.
        """
        if callback.from_user.id != self.mentor_id:
            await callback.answer("❌ У вас нет прав ментора!", show_alert=True)
    ...
    async def end_chat(self, message: types.Message):
        ...
        После завершения, если в очереди есть студенты, ментору отправляется уведомление.
        """
        if message.from_user.id != self.mentor_id:
